...
class DocumentService:
    ...
    async def analyze(
        ...
        await self._bus.emit(
            ...
            service=f"Content Understanding ({self._analyzer.provider})",
        )
        try:
            ...
            await self._bus.emit(
                ...
                status=EventStatus.failed, service="Content Understanding",
            )
            return self._repo.get()
        ...
        if not failing:
            await self._bus.emit(
                ...
                status=EventStatus.completed, service=f"Content Understanding ({result.provider})",
            )
        else:
            await self._bus.emit(
                event_type="document.review_required",
                label=f"Human review required ({len(failing)} field(s) below {int(CONFIDENCE_THRESHOLD*100)}%)",
                status=EventStatus.review, service=f"Content Understanding ({result.provider})",
            )
        return self._repo.get()
