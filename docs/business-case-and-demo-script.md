...
3. Open the synchronized customer and service views side by side. Explain that Emma uses the customer view while a Bank Alfa employee uses the service view.
4. Explain that the final mortgage decision remains with an advisor.

### Part 1: payslip understanding, 3 minutes

1. In the customer view, open the Income Document step.
2. Upload the bundled high-confidence Swedish payslip or choose it from the sample selector.
3. Show the customer's simple `We are reviewing your payslip` state, then move attention to the service view.
4. In the service view, point to the live `Content Understanding` activity and show each extracted field, its confidence, and its source highlight:
   - Northstar AB
   - SEK 96,000 gross
   - SEK 62,400 net
   - Permanent, full-time
   - 25 August 2026
5. Show that all required fields meet the `0.85` threshold and are saved to the mortgage case; the customer view changes to a simple accepted state.
6. Briefly switch to the bundled low-confidence sample, or reset and upload it, to show the review path.
7. On the service side, correct one uncertain field and approve it as the bank employee. Point out that automation pauses until approval and that the customer does not see internal confidence or tooling detail.
...
1. Presenter starts the voice session from the customer view while keeping the service view visible in a second window.
...
4. The agent calls `get_crm_profile`; the service view reveals Emma's profile and existing car loan. The customer view does not expose internal CRM data.
...
2. Show that the employee service view displays application-safe activity, while detailed platform traces remain available in Foundry.
...
- Every completed tool call appears in the service view's AI activity timeline.
- Customer-only and employee-only information remain on their respective sides.
