...
- Content Understanding: `azure-ai-contentunderstanding==1.1.0`, GA service API `2025-11-01`, long-running `begin_analyze` operation.
- Authentication: `azure-identity==1.25.3`, `DefaultAzureCredential`, and explicit `AZURE_CLIENT_ID` for the Container App's user-assigned identity.
- Telemetry: `azure-monitor-opentelemetry==1.8.9`.
...
2. Build `/service` as the bank-employee workspace for customer information, review, mortgage case, cards, and advisor summary.
3. Build document empty, analyzing, accepted, review, rejected, and failed states across the appropriate sides.
4. Build voice ready, identifying, active, interrupted, reconnecting, and ended states on the customer side.
